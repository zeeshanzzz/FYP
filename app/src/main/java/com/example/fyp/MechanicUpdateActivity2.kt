...
class MechanicUpdateActivity2 : AppCompatActivity() {
    lateinit var appDatabase: AppDatabase;
    override fun onCreate(savedInstanceState: Bundle?) {

        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_mechanic_update2)
        appDatabase= AppDatabase.getInstance(this@MechanicUpdateActivity2)!!
        ...
        btnCreateBusiness.setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View?) {
                ...
                InsertTask(this@MechanicUpdateActivity2,entity).execute()
            }
    ...
    inner  class InsertTask internal constructor(context: MechanicUpdateActivity2, note: MechanicEnity) : AsyncTask<Void?, Void?, Boolean>() {
        private val activityReference: WeakReference<MechanicUpdateActivity2>
...
//                activityReference.get()?.startActivity(intent)
        ...
        override fun doInBackground(vararg params: Void?): Boolean {
            ...
            try {
                activityReference.get()?.appDatabase?.mechanicDao()?.updateTodo(note);
                isInserted=true;
